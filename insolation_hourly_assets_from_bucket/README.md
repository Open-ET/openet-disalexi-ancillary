...
Update every day
```
gcloud scheduler jobs update http disalexi-insolation-hourly --schedule "5 22 * * *" --uri "https://us-central1-openet.cloudfunctions.net/disalexi-insolation-hourly" --description "DisALEXI Hourly Insolation Update" --http-method POST --time-zone "UTC" --project openet --location us-central1 --max-retry-attempts 1 --attempt-deadline=540s --min-backoff=30s
```
