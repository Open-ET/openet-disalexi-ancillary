...
ASSET_COLL_NAME = 'global_v001_hourly'
# ASSET_COLL_ID = 'projects/earthengine-legacy/assets/' \
#                 'projects/disalexi/meteo_data/{variable}/global_v001_hourly'
...
def hourly_ingest(tgt_dt, variable, overwrite_flag=False, delay_time=1):
    """
...
def hourly_dates(start_dt, end_dt, variable, overwrite_flag=False):
    """"""
    ...
    # logging.info(f'Task dates: {", ".join(task_date_list)}')

    # Switch date list to be dates that are missing
    test_dt_list = [
        dt for dt in test_dt_list
        if overwrite_flag or dt.strftime(ISO_DT_FMT) not in task_date_list]
    if not test_dt_list:
...
def hourly_date_range(start_dt, end_dt, hours=3, skip_leap_days=False):
...
def arg_parse():
    ...
    parser = argparse.ArgumentParser(
        description='Generate DisALEXI hourly meteo assets',
        formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument(
        '-v', '--variable', choices=VARIABLES, metavar='VAR',
        help='DisALEXI Meteo Variable')
    parser.add_argument(
...
if __name__ == '__main__':
    ...
    asset_coll_id = f'{ASSET_ROOT}/{ASSET_FOLDER[args.variable]}/{ASSET_COLL_NAME}'
    ...
    ingest_dt_list = hourly_dates(
        args.start, args.end, variable=args.variable, overwrite_flag=args.overwrite)
    ...
    for ingest_dt in sorted(ingest_dt_list, reverse=args.reverse):
        ...
        response = hourly_ingest(
            ingest_dt, args.variable, overwrite_flag=args.overwrite,
