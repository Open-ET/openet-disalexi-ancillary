...
```
gcloud functions deploy disalexi-meteo-hourly --project openet-dri --runtime python37 --entry-point cron_scheduler --trigger-http --allow-unauthenticated --memory 256 --timeout 540 --max-instances 1 --service-account="openet-dri@appspot.gserviceaccount.com"
```
...
```
gcloud scheduler jobs update http disalexi-meteo-hourly --schedule "52 7 * * *" --uri "https://us-central1-openet-dri.cloudfunctions.net/disalexi-meteo-hourly" --description "Update Hourly Meteo Assets" --http-method POST --time-zone "UTC" --project openet-dri --max-retry-attempts 5
```
